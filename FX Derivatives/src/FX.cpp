...
using namespace std;
result_data FX::evaluate_data_and_premium_barrier() const
{
    double dtau, alpha, k;
    ...
    int closestToSpotIndex_ = -1;
    double minDifference = __INT_MAX__;
    for (int i = 0; i < N; i++)
    {
        x[i] = xmin + i * dx;
        S[i] = K * exp(x[i]);
        ...
        if (abs(S[i] - S0) < minDifference)
        {
            minDifference = abs(S[i] - S0);
            closestToSpotIndex_ = i;
        }
    }
    for (int j = 0; j < M; j++)
    {
    ...
    result_data result(alpha, dtau, k, x, S, t, tau, u, v, closestToSpotIndex_);
    return result;
}
